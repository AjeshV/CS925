...
class HttpClient 
{
 static int number_of_iteration = 4;
  ...
  for (int i = 0; i < number_of_iteration; i++)
 {
   ...
   System.out.println("HTTP" + (i + 1) + ":    " + HTTPtime[i]);
 }
  System.out.println("Mean Time:" + MeanCalculation(HTTPtime));
  System.out.println("Standard Deviation: " + StdDevCalculation(HTTPtime));
}
static double sendGetRequest(String IP) throws Exception 
{
 URL url = new URL("http://["+IP+"]:"+HTTPport+"//~avv1004//index.html");
 System.out.println(url);
 double t1, t2, t3;
 t1 = System.nanoTime();
 HttpURLConnection httpcon = (HttpURLConnection) url.openConnection(); 
 httpcon.addRequestProperty("User", "Chrome");
